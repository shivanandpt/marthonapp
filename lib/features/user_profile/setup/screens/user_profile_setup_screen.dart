...
import 'package:marunthon_app/features/user_profile/setup/components/setup_step_indicator.dart';
import 'package:marunthon_app/features/user_profile/setup/components/basic_info_step.dart';
import 'package:marunthon_app/features/user_profile/setup/components/personal_details_step.dart';
import 'package:marunthon_app/features/user_profile/setup/components/training_pref_step.dart';
import 'package:marunthon_app/features/user_profile/setup/components/health_info_step.dart';
...
class _UserProfileSetupScreenState extends State<UserProfileSetupScreen> {
    ...
    return ChangeNotifierProvider.value(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          elevation: 0,
          // Add skip button as action - ONLY show after Basic Info (step 0)
          actions: [
            Consumer<UserProfileSetupController>(
              builder: (context, controller, _) {
                // Only show Skip button if NOT on Basic Info page (step 0)
                if (controller.currentStep > 0) {
                  return TextButton(
                    onPressed: () {
                      // Skip to the last page (Health Information)
                ...
                return SizedBox.shrink(); // Don't show Skip on Basic Info
              },
            ),
          ],
        ),
    ...
    if (controller.currentStep < controller.totalSteps - 1) {
      ...
      if (success) {
        if (controller.isEditMode) {
          // For edit mode, just go back to previous screen
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Profile updated successfully!')),
          );
          context.pop(); // Use GoRouter
        } else {
          // For new users, navigate to completion screen using GoRouter
          context.go(
            '/setup-complete?name=${Uri.encodeComponent(controller.name)}',
          );
        }
      } else {
        // Show error
  ...
  // Helper method to get step titles
